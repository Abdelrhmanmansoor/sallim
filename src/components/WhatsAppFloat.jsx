...
export default function WhatsAppFloat() {
  ...
  return (
    <div className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 z-40 flex items-end gap-3 pointer-events-none" style={{ right: 'auto' }}>
      {/* Message Bubble */}
      {isDesktop && (
        <div
          className={`hidden sm:block relative bg-white rounded-2xl rounded-br-md p-4 shadow-2xl shadow-black/20 max-w-[280px] pointer-events-auto transition-all duration-400 ease-out ${showMessage ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95 pointer-events-none'
            }`}
        >
          {/* Close button */}
          <button
          ...
          {/* Bubble tail */}
          <div className="absolute bottom-4 -left-2 w-4 h-4 bg-white transform rotate-45 pointer-events-none shadow-sm" style={{ zIndex: -1 }} />
        </div>
      )}
