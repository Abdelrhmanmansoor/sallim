...
export default function WhatsAppFloat() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (!dismissed) setShowMessage(true)
    }, 3000)
  ...
  return (
    <div className="fixed bottom-6 left-6 z-50 flex items-end gap-3">
      ...
      <div 
        className={`
          relative bg-white rounded-2xl rounded-bl-md p-4 shadow-2xl shadow-black/20
          max-w-[280px] transition-all duration-500 ease-out
          ${showMessage ? 'opacity-100 translate-x-0 scale-100' : 'opacity-0 -translate-x-4 scale-95 pointer-events-none'}
        `}
      >
        {/* Close button */}
        <button 
        ...
        {/* Discount badge */}
        <div className="absolute -top-3 left-4 bg-red-500 text-white text-[11px] font-black px-2.5 py-1 rounded-full animate-pulse">
          خصم 70%
        </div>
        ...
        {/* Bubble tail */}
        <div className="absolute bottom-3 -left-2 w-4 h-4 bg-white transform rotate-45" />
      </div>
      
      {/* WhatsApp Button */}
      <button
        onClick={handleWhatsAppClick}
        className="group relative w-14 h-14 bg-[#25D366] hover:bg-[#20bd5a] rounded-full shadow-lg shadow-[#25D366]/30 hover:shadow-xl hover:shadow-[#25D366]/40 transition-all duration-300 hover:scale-110 flex items-center justify-center"
      >
